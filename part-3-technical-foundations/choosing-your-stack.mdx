...
**Data transformations in the warehouse**. If you're using dbt for data transformations (covered in Chapter 18), you'll write Python for custom models that run in Snowpark (Snowflake), BigQuery DataFrames, or PySpark. The data warehouse ecosystem is Python-native.
...
**The exception**: If you have specific compliance requirements (Chapter 21 covers security and compliance), you might need self-hosted infrastructure for data residency or audit reasons. But this is rare for most funds and work with your CISO or compliance officer to find a path forward. Default to managed services.
...
- A validation library (Zod for TypeScript, covered in Chapter 20) is essential for API integrations
