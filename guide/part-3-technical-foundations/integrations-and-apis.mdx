...
This chapter covers common integration patterns, validation strategies, webhook handling, and rate limits. The focus is on practical patterns that work for VC funds.
...
**Return quickly**: Webhook handlers should return 200 OK quickly (within a few seconds). Don't do expensive processing in the webhook handler itself. Accept the webhook, queue the work (in a job queue like [BullMQ](https://bullmq.io/) or background task system), and return success. Process the work asynchronously.
...
- **[Dagster](https://dagster.io/)**: Data orchestration, can schedule file imports and transformations
...
Much of the technical work at VC funds is glue code. Validate everything with Zod or Pydantic: never trust external APIs. Respect rate limits with exponential backoff. Use database transactions to avoid partial failures.

For LLMs, use an AI gateway for failover. Pay attention to vendor API changes - they happen frequently.
