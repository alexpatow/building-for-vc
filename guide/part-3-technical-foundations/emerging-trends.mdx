...
**Don't fine-tune models**. Foundation models work fine for every VC use case (extracting data, analyzing companies, answering questions). Fine-tuning requires training data, evaluation infrastructure, ongoing maintenance, and rarely produces better results for most VC tasks.
...
**Model selection is simpler than you think**. We use Claude Sonnet for most tasks, Claude Opus for deep reasoning. Don't optimize for tiny cost differences (Sonnet is $3/million tokens - engineering time costs more). Pick an LLM provider, stick with it, and focus on building features.
