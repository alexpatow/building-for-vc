...
This chapter covers what data you need, which vendors provide it, how they deliver it, costs, and how to use them effectively.
...
- Test accuracy with your portfolio companies ([Data Quality](/guide/part-3-technical-foundations/data-quality))
...
- Rate limits (covered in [Integrations and APIs](/guide/part-3-technical-foundations/integrations-and-apis))
- Per-request or per-entity costs (each API call might cost money)
- Need to handle failures, retries, validation ([Integrations and APIs](/guide/part-3-technical-foundations/integrations-and-apis))
...
As covered in [Data Quality](/guide/part-3-technical-foundations/data-quality), some vendors prioritize accuracy (verify information before publishing, resulting in lag), others prioritize speed (publish quickly, may have more errors). PitchBook is accurate but slow. Signal providers are fast but incomplete.
...
**Coverage differences**: Vendors excel at different stages (Harmonic/Specter for early-stage, PitchBook for established), geographies (strong US vs weak international), and sectors (deep tech, bio, fintech).

**Test with portfolio companies**: As covered in [Data Quality](/guide/part-3-technical-foundations/data-quality), evaluate vendor accuracy against your portfolio companies to understand what each vendor is good for.

**Data freshness**: Vendors update at different cadences (real-time, daily, weekly, quarterly). Know how fresh the data is and don't present stale data as current.
...
**Per-request or per-entity**: You pay for each API call or each entity returned. A person data provider might charge $0.01 per person record. This is predictable per query but can add up quickly with high usage.

**Subscriptions**: Pay a fixed amount per year for unlimited (or high-limit) access. Common for established vendors. Might be $10,000-100,000+ per year depending on the vendor and what you're accessing.
...
Data costs can easily exceed infrastructure costs (servers, databases, etc.). Budget $10-15k/year for data subscriptions at a small fund, much more at larger funds with comprehensive data needs. Factor this into your overall technology budget from the start.
...
Use validation (Zod, Pydantic - covered in [Integrations and APIs](/guide/part-3-technical-foundations/integrations-and-apis)) to catch schema changes early. When a vendor changes their API, your validation will fail and alert you immediately, rather than silently corrupting your data.
